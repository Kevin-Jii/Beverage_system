              ...
              <a-scrollbar v-show="activeKey==1" style="overflow: auto;" :style="{height:`${windHeight}px`}">
                <div class="besecontent" >
                  <a-row :gutter="16">
										...
										<a-col :span="12">
											<a-form-item field="nickename" label="昵称" :rules="[{required:true,message:'请填写昵称'}]" >
													<a-input v-model="formData.nickename" placeholder="请填昵称" />
											</a-form-item>
										</a-col>
										<a-col :span="12">
										<a-form-item field="file_link" label="附件" style="margin-bottom:15px;">
										<a-upload
											accept=".zip,.rar"
											...
											<template #upload-button>
												...
												<div class="showfile">
													<a v-if="formData.file" href="formData.file" download="源码文件"><icon-upload /> 已上传代码包</a>
                    ...
                    <a-col :span="12">
										...
										<a-col :span="12">
											<a-form-item field="des" label="备注" :rules="[{required:true,message:'请填写备注'}]" >
													<a-textarea v-model="formData.des" placeholder="请填备注" :auto-size="{minRows:3,maxRows:5}"/>
											</a-form-item>
										</a-col>
                  </a-row>
                </div>
...
<script lang="ts">
  ...
  //api
  import { save,getContent } from '@/api/createcode/api';
          ...
          const controller = new AbortController();
            (async function requestWrap() {
              ...
              try {
                ...
                //更新附件
                if(resdata){
                  formData.value.file=resdata.url
                }

      ...
      <a-modal v-model:visible="AddTypeVisible" @cancel="AddTypeVisible=false" @ok="handleTypeSubmit" :closable="false">
        <a-form ref="typeformRef" :model="typeForm" auto-label-width>
          ...
          <template v-if="typeForm.isself==0" >
            ...
            <a-form-item  field="user_tablename" label="授权用户数据表名" extra="用于测试的用户账号所在的表" validate-trigger="input" style="margin-bottom:15px;">
...
<script lang="ts">
  ...
  import { getTables,TableItem,getRoutes,getTablelist,UserItem} from '@/api/developer/devapi';
    ...
    setup(_, { emit }) {
     ...
     const handleSubmit = async () => {
      ...
      return { 
        ...
        tablelist,routerlist,userlist,onSelectUserList,handleSearch,userloading,
      };
    },
  });
</script>

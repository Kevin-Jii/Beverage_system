<template>
  <BasicModal v-bind="$attrs" @register="registerModal" :footer="false" :isPadding="false" :loading="loading" width="1000px" @height-change="onHeightChange" :minHeight="modelHeight" :title="getTitle" @ok="handleSubmit">
                        ...
                        <a-form-item label="图标" field="icon" style="margin-bottom:15px;">
                          <a-input-search v-model="formData.icon" placeholder="选择图标/填写"  search-button>
                            <template  v-if="formData.icon" #prefix>
                              <Icon :icon="formData.icon" :size="32"></Icon>
                            </template>
                    ...
                    <a-col :span="12">
                      <a-form-item field="tpl_type" label="模板类型" validate-trigger="input" :rules="[{required:true,message:'请选择模板类型'}]" >
                        <a-select @change="handelTplType" v-model="formData.tpl_type" placeholder="选择模板类型" allow-clear >
                            <a-option  value="list">仅数据列表</a-option>
                            <a-option  value="cate">数据关联分类</a-option>
                          </a-select>
                      </a-form-item>
                    </a-col>
                    <a-col :span="12" v-if="formData.tpl_type=='cate'">
